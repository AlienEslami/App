    ...
    for sheet_name in sheet_names:
        if sheet_name in workbook.sheetnames:
            ...
            for row in rows[1:]:
                ...
                records.append(record)
            return records
    raise ValueError(f'None of the expected sheets {sheet_names} were found in {workbook_path}')
...
def build_input_data(workbook_path: Path, spot_prices_path: Path | None = None,
    ...
    spot_prices = load_first_available_sheet_records(spot_source_path, ['Spot Prices', 'Prices'])
...
def update_realtime_sheet(workbook, sc: dict, model, timestep: int) -> None:
    ...
    for bus_idx in range(1, sc['k_count'] + 1):
        ...
        status, trip_idx, charger_idx = status_for_bus(model, bus_idx, timestep, trip_count, charger_count)
...
def generate_benchmark_files(template_path: Path, output_dir: Path,
                             ...
                             tariffs_path: Path | None = None,
                             summary_workbook: Path | None = None,
                             reasoning_source: str | None = None,
                             reasoning_text: str | None = None) -> Path:
    ...
    summary_target = summary_workbook or template_path
    summary_row = build_day_ahead_summary_row(
        sc,
        model,
        'aggregator_smart_charging_v2g',
        input_workbook=template_path,
        spot_prices_file=spot_prices_path,
        tariffs_file=tariffs_path,
    )
    append_day_ahead_summary(summary_target, summary_row)
    ...
    append_agent_reasoning(summary_target, reasoning_row)
    ...
    summary_lines = [
        ...
        f'summary_workbook={summary_target}',
        'summary_sheet=day_ahead_summary',
    ]
